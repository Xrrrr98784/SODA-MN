biological.data.preparation<-function(data.path,code.path,file.name){
#read ms2 spectra for biological replicates

library(CluMSID)
#file.name="Mets_gut_mix_control_extra"
path <- paste(data.path,file.name,sep="")
setwd(path)
file.list <- grep(pattern = "mzXML", dir(path), value = TRUE)
